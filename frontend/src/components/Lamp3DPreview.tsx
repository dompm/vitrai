    ...
    >
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text-soft)' }}>
            Geometry Engine
          </span>
          {/* Preset Buttons */}
          <div style={{ display: 'flex', gap: 4 }}>
            {(['cylinder', 'cone', 'dome', 'pyramid'] as const).map(preset => (
              <button
                key={preset}
                className="btn-ghost"
        ...
        {/* N Facets Count Slider */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <span style={{ fontSize: '12px', width: 100, color: 'var(--text-soft)' }}>
            Facets (N):
          </span>
          <input
            type="range"
            ...
            max={32}
            value={N}
            onChange={e => onUpdateLampConfig({ facetCount: parseInt(e.target.value) })}
            style={{ flex: 1, accentColor: 'var(--amber)' }}
          />
          <span style={{ fontSize: '12px', fontWeight: 600, width: 60, textAlign: 'right', color: 'var(--text-bright)' }}>
            {N === 32 ? 'Smooth' : N}
          </span>
        </div>

        {/* Profile + Tier Selector Workspace Layout */}
        <div style={{ display: 'flex', gap: 14 }}>
          {/* SVG Profile Editor */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            <span style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.03em', color: 'var(--text-dim)' }}>
              Side Profile Curve
            </span>
            <div style={{ background: 'var(--paper)', border: '1px solid var(--hairline-2)', borderRadius: '6px', overflow: 'hidden', height: 215, width: 160 }}>
              <svg
                ref={svgRef}
                onClick={handleSvgBgClick}
                style={{ width: '100%', height: '100%', cursor: 'crosshair', display: 'block' }}
              >
                {/* Y Axis line at R=0 (Offset by 20px) */}
                <line x1={20} y1={0} x2={20} y2={215} stroke="var(--hairline-2)" strokeWidth={1} strokeDasharray="3,3" />
                ...
                {/* Profile Path Line */}
                <path d={profileLinePath} fill="none" stroke="var(--amber)" strokeWidth={2.5} />
                ...
                {profilePoints.map((pt, i) => {
                  ...
                  return (
                    <circle
                      ...
                      cy={15 + pt.y * 0.8}
                      r={isSelected ? 6 : 4.5}
                      fill={isSelected ? 'var(--amber-ink)' : 'var(--paper)'}
                      stroke="var(--amber)"
          ...
          {/* Point Coordinates Panel & Tier Tabs */}
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              <span style={{ fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.03em', color: 'var(--text-dim)' }}>
                Active Segment/Tier
              </span>
              <div style={{ display: 'flex', gap: 2, flexWrap: 'wrap', background: 'var(--cream-soft)', padding: 3, borderRadius: 5 }}>
                {profilePoints.slice(0, -1).map((_, t) => (
                  <button
                    key={t}
                    onClick={() => onUpdateLampConfig({ activeTierIndex: t })}
                    style={{
                      flex: 1,
                      border: 'none',
                      borderRadius: '4px',
                      ...
                      fontSize: '11.5px',
                      cursor: 'pointer',
                      background: t === activeTierIndex ? 'var(--paper)' : 'transparent',
                      color: t === activeTierIndex ? 'var(--amber-ink)' : 'var(--text-soft)',
                      fontWeight: t === activeTierIndex ? 600 : 500,
                      boxShadow: t === activeTierIndex ? '0 1px 2px rgba(0,0,0,0.06)' : 'none',
                    }}
                  >
                    Tier {t + 1}
                  </button>
            ...
            {/* Selected Control Point Coordinates Input */}
            {selectedPointIdx !== null && selectedPointIdx < profilePoints.length && (
              <div
                style={{
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-bright)' }}>
                    Point {selectedPointIdx + 1} ( {selectedPointIdx === 0 ? 'Top' : selectedPointIdx === profilePoints.length - 1 ? 'Base' : 'Middle'} )
                  </span>
                  {profilePoints.length > 2 && (
                    <button
                      onClick={() => handleDeleteProfilePoint(selectedPointIdx)}
                      style={{
                        background: 'none',
                        border: 'none',
                        color: 'var(--ruby)',
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  ...
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2 }}>
                    <span style={{ fontSize: '10px', color: 'var(--text-soft)' }}>Radius (R)</span>
                    <input
                      type="number"
                      value={profilePoints[selectedPointIdx].r}
                      onChange={e => {
                        const val = Math.max(0, parseInt(e.target.value) || 0);
                        const updated = [...profilePoints];
                        updated[selectedPointIdx] = { ...updated[selectedPointIdx], r: val };
                        onUpdateLampConfig({ profilePoints: updated });
                      }}
                      style={{
                        width: '100%',
                  ...
                  {/* Height Input (Only editable if not top point) */}
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2 }}>
                    <span style={{ fontSize: '10px', color: 'var(--text-soft)' }}>Height (Y)</span>
                    <input
                      type="number"
                      value={profilePoints[selectedPointIdx].y}
                      disabled={selectedPointIdx === 0}
                      onChange={e => {
                        ...
                        const minY = updated[selectedPointIdx - 1].y + 15;
                        const maxY = selectedPointIdx < updated.length - 1 ? updated[selectedPointIdx + 1].y - 15 : 400;
                        const clamped = Math.min(Math.max(val, minY), maxY);

                        updated[selectedPointIdx] = { ...updated[selectedPointIdx], y: clamped };
                        onUpdateLampConfig({ profilePoints: updated });
                      }}
                      style={{
                        width: '100%',
                        ...
                        borderRadius: '4px',
                        fontSize: '12px',
                        background: selectedPointIdx === 0 ? 'transparent' : 'var(--paper)',
                        color: 'var(--text-bright)',
                      }}
                    />
                  </div>
