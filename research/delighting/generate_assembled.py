...
def build_flat_scene(scene, hdri_path, recipe, sample_dir, seed, uniform=False):
    ...
    (img_T, img_h, img_mark, img_mark_white, img_mark_index, img_height, img_normal, bump_distance,
     img_sigma_s, img_a_glow) = create_glass_textures(recipe, sample_dir, size=1536, seed=seed)
    ...
    # report 037 item B; img_sigma_s/img_a_glow added report 043 MMv3-G1)
    # after report 014 was authored; pass all 9 so the caller matches the
    # current generate_synthetic.py (not edited here -- see module
    # docstring).
