#!/usr/bin/env python3
"""Two-frame same-background motion-constrained inverse experiment.

Report 022 showed that two unrelated learned backgrounds barely help. This
variant makes the second frame informative:

  obs1 = render(T, B, D)
  obs2 = render(T, B, D + known_shift)

So T, D, and B are all shared. Only the sampling coordinate of the background
changes. If this helps, the right capture/research primitive is not "two random
photos"; it is known relative motion.
"""
import argparse
import json
import os
...
OUT_DEFAULT = os.path.join(HERE, "results", "differentiable_sheet_motion")
...
def shift_field(n, shift_xy):
    d = np.zeros((n, n, 2), np.float32)
    d[..., 0] = float(shift_xy[0])
    d[..., 1] = float(shift_xy[1])
    return d


def render_np(T, B, D, ambient, leak, rng=None):
...
def optimize_motion(obs_list, shifts, args, device):
    n = obs_list[0].shape[0]
    obs_t = [inv.np_to_torch_img(obs, device) for obs in obs_list]
    shift_t = [
        torch.from_numpy(shift_field(n, s)).permute(2, 0, 1)[None].float().to(device)
        for s in shifts
    ]
    ...
    return {"T": T_np, "B": B_np, "disp": disp_np, "log": log}


def motion_metrics(name, rec, gt, args):
    recon = []
    for obs, shift in zip(gt["obs"], gt["shifts"]):
        warped = inv.warp_np(rec["B"], rec["disp"] + shift_field(gt["T"].shape[0], shift))
        pred = np.clip(rec["T"] * (args.ambient + args.leak * warped), 0, 1)
        recon.append(float(np.mean(np.abs(pred - obs))))
    lum_T = (rec["T"] * inv.LUM).sum(-1)
    lum_gt = (gt["T"] * inv.LUM).sum(-1)
    bg_lum = (gt["warped"][0] * inv.LUM).sum(-1)
    return {
        "name": name,
        "renderer_recon_mae": float(np.mean(recon)),
        "T_mae": float(np.mean(np.abs(rec["T"] - gt["T"]))),
        "T_lum_mae": float(np.mean(np.abs(lum_T - lum_gt))),
        "B_mae": float(np.mean(np.abs(rec["B"] - gt["B"]))),
        "preview_lum_cv": inv.patch_lum_cv(rec["T"]),
        "T_highfreq_corr_with_bg": inv.corr(inv.highpass(lum_T), inv.highpass(bg_lum)),
...
def save_contact(out_dir, gt, single, motion):
    rows = []
    first = [
    ...
    for rec, label in ((single, "single learnedB"), (motion, "motion sharedB")):
...
def write_summary(out_dir, metrics):
    lines = [
        "# Motion-constrained inverse summary",
        "",
        "| method | recon MAE | T MAE | B MAE | preview lum CV | T-bg corr | disp EPE |",
        "|---|---:|---:|---:|---:|---:|---:|",
    ]
    for m in metrics:
        lines.append(
            f"| {m['name']} | {m['renderer_recon_mae']:.4f} | {m['T_mae']:.4f} | {m['B_mae']:.4f} | "
            f"{m['preview_lum_cv']:.3f} | {m['T_highfreq_corr_with_bg']:.3f} | {m['disp_epe_px']:.2f} |"
        )
...
def run_case(args, out_dir, case_name="single"):
    ...
    for shift, rr in zip(shifts, (rng, rng2)):
        o, w = render_np(T, B, D + shift_field(args.size, shift), args.ambient, args.leak, rr)
        obs.append(o)
        warped.append(w)

    t0 = time.time()
    single_raw = inv.optimize_lowT_disp_B(obs[0], args, device)
    single = {"T": single_raw["T"], "B": single_raw["bg"], "disp": single_raw["disp"], "log": single_raw["log"]}
    motion = optimize_motion(obs, shifts, args, device)
    elapsed = time.time() - t0

    gt = {"T": T, "D": D, "height": height, "B": B, "obs": obs, "warped": warped, "shifts": shifts}
    metrics = [
        motion_metrics("single-frame learned B", single, gt, args),
        motion_metrics("two-frame shared B with known shift", motion, gt, args),
    ]
    save_contact(out_dir, gt, single, motion)
    payload = {
        "case": case_name,
        "claim": "Known relative motion with shared B may reduce T/B ambiguity.",
        "config": vars(args) | {"device": device, "elapsed_s": elapsed, "shifts": shifts},
        "metrics": metrics,
        "logs": {"single": single_raw["log"], "motion": motion["log"]},
    }
    with open(os.path.join(out_dir, "metrics.json"), "w") as f:
        json.dump(payload, f, indent=2)
    write_summary(out_dir, metrics)
    print(f"==== MOTION INVERSE: {case_name} ====")
    for m in metrics:
        print(
            f"{m['name']:34s} recon={m['renderer_recon_mae']:.4f} T={m['T_mae']:.4f} "
...
def summarize_sweep(out_dir, cases):
    ...
    summary = {}
    lines = [
        "# Motion-constrained inverse sweep",
        "",
        "| method | T MAE mean | T MAE std | B MAE mean | preview CV mean | T-bg corr mean | disp EPE mean | n |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for method, rows in sorted(by_method.items()):
        ...
        lines.append(f"| {method} | {t_mu:.4f} | {t_sd:.4f} | {b_mu:.4f} | {cv_mu:.3f} | {corr_mu:.3f} | {epe_mu:.2f} | {len(rows)} |")
...
def main():
    ...
    ap.add_argument("--sweep-seeds", default="41,42,43,44")
    ...
    print("==== MOTION SWEEP SUMMARY ====")
    for method, row in summary.items():
        print(f"{method}: T={row['T_mae_mean']:.4f} B={row['B_mae_mean']:.4f} corr={row['T_highfreq_corr_with_bg_mean']:.3f}")
