#!/usr/bin/env python3
"""Soft integrability regularization for two-frame sheet inverse rendering.

Report 025 showed that projecting free-flow displacement into a scalar relief
field modestly improves geometry/leakage but leaves a gap to oracle relief. This
script tests whether the projection's useful ingredient can be moved inside the
optimizer as a curl penalty:

  curl(D) = dD_y/dx - dD_x/dy

Gradient fields have zero curl. A soft penalty may keep free-flow optimization
easy while discouraging nonphysical local warps before they become baked in.
"""
import argparse
import json
import os
...
OUT_DEFAULT = os.path.join(HERE, "results", "differentiable_sheet_curl_regularized_sweep")
...
def ddx(a):
    return F.pad((a[..., 2:] - a[..., :-2]) * 0.5, (1, 1, 0, 0), mode="replicate")


def ddy(a):
    return F.pad((a[..., 2:, :] - a[..., :-2, :]) * 0.5, (0, 0, 1, 1), mode="replicate")


def curl_torch(disp):
    return ddx(disp[:, 1:2]) - ddy(disp[:, 0:1])


def curl_abs_np(disp):
    dx_y = np.pad((disp[:, 2:, 1] - disp[:, :-2, 1]) * 0.5, ((0, 0), (1, 1)), mode="edge")
    dy_x = np.pad((disp[2:, :, 0] - disp[:-2, :, 0]) * 0.5, ((1, 1), (0, 0)), mode="edge")
    return float(np.mean(np.abs(dx_y - dy_x)))


def optimize_motion_curl(obs_list, shifts, args, device, curl_weight):
    ...
    for step in range(args.steps_motion):
        ...
        curl_loss = curl_torch(disp / args.max_disp).abs().mean()
        loss = (
            loss_recon
            + args.t_tv * inv.tv(T)
            + args.b_tv * inv.tv(B)
    ...
    return {"T": T_np, "B": B_np, "disp": disp_np, "curl_weight": curl_weight, "log": log}


def method_metrics(name, rec, gt, args):
    row = motion.motion_metrics(name, rec, gt, args)
    row["curl_abs"] = curl_abs_np(rec["disp"] / args.max_disp)
    row["curl_weight"] = float(rec.get("curl_weight", 0.0))
    return row
...
def save_contact(out_dir, gt, methods):
    first = [
        ...
        hf.heat_tile(curl_map_np(gt["D"] / gt["max_disp"]), "GT curl"),
    ]
    rows = []
    ...
    for label, rec in methods:
        ...
        row = [
            ...
            hf.heat_tile(curl_map_np(rec["disp"] / gt["max_disp"]), f"{label} curl"),
        ]
...
def write_summary(out_dir, metrics):
    ...
    for m in metrics:
        lines.append(
            f"| {m['name']} | {m['curl_weight']:.4f} | {m['renderer_recon_mae']:.4f} | "
            f"{m['T_mae']:.4f} | {m['B_mae']:.4f} | {m['preview_lum_cv']:.3f} | "
            f"{m['T_highfreq_corr_with_bg']:.3f} | {m['disp_epe_px']:.2f} | {m['curl_abs']:.5f} |"
        )
...
def run_case(args, out_dir, case_name="single"):
    ...
    t0 = time.time()
    methods = []
    for weight in parse_floats(args.curl_weights):
        rec = optimize_motion_curl(obs, shifts, args, device, weight)
        methods.append((f"curl {weight:g}", rec))
    ...
    payload = {
        "case": case_name,
        "claim": "Soft curl penalty may preserve easy free-flow fitting while encouraging physical/integrable refraction.",
    ...
    print(f"==== CURL REGULARIZED: {case_name} ====")
    for m in metrics:
        print(
            f"{m['name']:14s} recon={m['renderer_recon_mae']:.4f} T={m['T_mae']:.4f} "
            f"B={m['B_mae']:.4f} corr={m['T_highfreq_corr_with_bg']:.3f} "
            f"dispEPE={m['disp_epe_px']:.2f} curl={m['curl_abs']:.5f}"
        )
...
def summarize_sweep(out_dir, cases):
    ...
    for method, rows in sorted(by_method.items(), key=lambda kv: kv[1][0]["curl_weight"]):
        ...
        curl_mu, _ = mean_std(rows, "curl_abs")
        curl_w = float(rows[0]["curl_weight"])
        summary[method] = {
            "curl_weight": curl_w,
            "T_mae_mean": t_mu,
            "T_mae_std": t_sd,
            ...
            "curl_abs_mean": curl_mu,
            "n": len(rows),
        }
        lines.append(
            f"| {method} | {curl_w:.4f} | {t_mu:.4f} | {t_sd:.4f} | {b_mu:.4f} | "
            f"{cv_mu:.3f} | {corr_mu:.3f} | {epe_mu:.2f} | {curl_mu:.5f} | {len(rows)} |"
        )
...
def main():
    ...
    ap.add_argument("--curl-weights", default="0,0.03,0.10,0.30")
    ap.add_argument("--log-every", type=int, default=450)
    ap.add_argument("--sweep", action="store_true")
    ap.add_argument("--sweep-seeds", default="71,72,73,74")
    ...
    print("==== CURL SWEEP SUMMARY ====")
    for method, row in summary.items():
        print(
            f"{method}: T={row['T_mae_mean']:.4f} B={row['B_mae_mean']:.4f} "
            f"corr={row['T_highfreq_corr_with_bg_mean']:.3f} curl={row['curl_abs_mean']:.5f}"
        )
