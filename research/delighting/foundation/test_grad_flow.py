...
# 053b port note: `sigma_s` added (report 048 extended the aux to 8 channels; the head set
# and the synthetic batch must cover EVERY supervised head or a severed one hides again).
HEADS = ("T", "h", "sigma_s", "B", "shadow", "mark", "conf")
...
def _synthetic_batch(device, size=64, seed=0):
    ...
    batch = {
        ...
        "h": rnd(1), "sigma_s": rnd(1), "shadow": (rnd(1) > 0.5).float(), "mark": mark,
        "valid": 1.0 - mark,
    }
    batch["has_B"] = torch.tensor([1.0], device=device)
    batch["has_sigma_s"] = torch.tensor([1.0], device=device)
    return batch
